...
export const getSubCenters = async () => {
  ...
  return apiClient.get('/api/v1/subcenters/');
...
export const SubCenterService = {
  createSubCenter: async (data: any) => {
    return apiClient.post('/api/v1/subcenters/', data); // Verify path
  },
  updateSubCenter: async (id: number, data: any) => {
    return apiClient.patch(`/api/v1/subcenters/${id}/`, data); // Verify path
  },
};