...
# Set all environment variables at once for better performance
$allSettings = @(
    ...
    "DB_PORT=5432",
    "DJANGO_SECRET_KEY=parliament-fuel-secret-$((Get-Random))-$((Get-Date -Format 'yyyyMMddHHmmss'))",
    "DJANGO_DEBUG=False",
...
Write-Host "Setting all environment variables..." -ForegroundColor White

# Convert settings array to arguments for az command
$settingsArgs = $allSettings -join " "

# Execute the Azure CLI command
$command = "az webapp config appsettings set --resource-group $resourceGroup --name $appName --settings $settingsArgs"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "Some environment variables may have failed to configure." -ForegroundColor Yellow
    }
} catch {
    Write-Host "Error configuring environment variables: $_" -ForegroundColor Red
}
