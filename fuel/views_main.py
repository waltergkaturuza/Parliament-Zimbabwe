...
@api_view(['GET'])
@permission_classes([IsAuthenticated, AdminPermission | AuditorPermission])
def audit_compliance_stats(request):
    ...
    try:
        ...
        stats = {
            ...
            'successful_actions': audit_logs.filter(success=True).count(),
        ...
        if stats['total_actions'] > 0:
            stats['compliance_rate'] = round((stats['successful_actions'] / stats['total_actions']) * 100, 2)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated, AdminPermission | AuditorPermission])
def audit_compliance_reports(request):
        ...
        reports = [
            {
                'id': 1,
                'title': 'Monthly Compliance Report',
                'description': 'System compliance overview for the past month',
                'status': 'completed',
                'generated_date': timezone.now().strftime('%Y-%m-%d'),
            ...
            },
            {
                ...
                'status': 'in_progress',
                'generated_date': (timezone.now() - timedelta(days=1)).strftime('%Y-%m-%d'),
                'compliance_score': 87.2,
            }
        ]
        
        return Response({'reports': reports, 'period': period})
    except Exception as e:
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated, AdminPermission | AuditorPermission])
def audit_transaction_stats(request):
    """Audit transaction statistics"""
    try:
        # Get transaction stats from actual data
        transactions = FuelTransaction.objects.all()
        
        stats = {
            'total_transactions': transactions.count(),
            'approved_transactions': transactions.filter(approved=True).count(),
            'pending_transactions': transactions.filter(approved=False).count(),
            'total_volume': float(transactions.aggregate(total=Sum('litres'))['total'] or 0),
            'average_transaction_size': 0,
            'last_updated': timezone.now().strftime('%Y-%m-%d %H:%M:%S'),
        }
        
        if stats['total_transactions'] > 0:
            stats['average_transaction_size'] = round(stats['total_volume'] / stats['total_transactions'], 2)
...
def programs_view(request):
    """Parliament programs/sessions data"""
    try:
        # Get actual parliament sessions as programs
        programs = ParliamentSession.objects.all()
        
        program_data = []
        for session in programs:
            program_data.append({
                'id': session.id,
                'name': session.session_name,
                'description': f"Parliament session: {session.session_name}",
                ...
                'participants_count': SessionAttendance.objects.filter(session=session).count(),
            })
        
        return Response({'programs': program_data})
    except Exception as e:
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def financial_analytics_view(request):
    ...
    try:
        ...
        transactions = FuelTransaction.objects.filter(transaction_date__range=[start_date, end_date])
        fuel_data = FuelData.objects.filter(created__date__range=[start_date, end_date])
        
        total_cost = 0
        total_litres = 0
        
        for transaction in transactions:
            if transaction.litres and transaction.price_per_litre:
                cost = float(transaction.litres) * float(transaction.price_per_litre)
                total_cost += cost
                total_litres += float(transaction.litres)
        
        # Get fuel price data
        avg_petrol_price = fuel_data.aggregate(avg=Avg('petrol_price'))['avg'] or 0
        avg_diesel_price = fuel_data.aggregate(avg=Avg('diesel_price'))['avg'] or 0
        ...
        analytics = {
            ...
            'summary': {
                ...
                'average_cost_per_litre': round(total_cost / total_litres, 2) if total_litres > 0 else 0,
                'total_transactions': transactions.count(),
            },
            'fuel_prices': {
                'average_petrol_price': round(float(avg_petrol_price), 2),
                'average_diesel_price': round(float(avg_diesel_price), 2),
            },
            'trends': {
                'daily_cost': [],
                'daily_volume': [],
...
def fuel_prices_view(request):
    """Current fuel prices"""
    try:
        latest_fuel_data = FuelData.objects.latest('created')
        
        prices = {
            'petrol_price': float(latest_fuel_data.petrol_price) if latest_fuel_data.petrol_price else 0,
            'diesel_price': float(latest_fuel_data.diesel_price) if latest_fuel_data.diesel_price else 0,
            'last_updated': latest_fuel_data.created.strftime('%Y-%m-%d %H:%M:%S'),
            'currency': 'USD',
        }
